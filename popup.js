...
function renderEntryList() {
  ...
  for (const e of sorted) {
    ...
    if (editingEntryId && e.id === editingEntryId) {
      ...
      timeInput.addEventListener("keydown", onEnterSave);
      textInput.addEventListener("keydown", onEnterSave);
...
function setEntrySortOrder(order) {
...
async function onSaveEntry(sessionId, entryId, text, timeRaw) {
  ...
  if (sec == null) {
    window.alert("시간 형식이 올바르지 않습니다. HH:MM:SS, MM:SS, 또는 초(숫자)로 입력하세요.");
    return;
  }
 ...
 * 팝업을 연 채로 다른 VOD로 넘어가도 목록이 예전 세션에 고정되지 않게 함.
 */
async function applyActivePageContextSelection(ctx) {
  ...
  if (ctx.resolvedSessionId) {
    target = sessions.find((s) => s.sessionId === ctx.resolvedSessionId);
  }
  if (!target && ctx.vodId) {
  ...
  if (!target && ctx.liveId) {
    target = sessions.find((s) => s.sessionId === `live:${ctx.liveId}`);
  }
  if (!target) {
...
async function init() {
  ...
  sortDescCheck?.addEventListener("change", () => {
    if (sortDescCheck.checked) setEntrySortOrder("desc");
    else setEntrySortOrder("asc");
  });
  sortAscCheck?.addEventListener("change", () => {
    if (sortAscCheck.checked) setEntrySortOrder("asc");
    else setEntrySortOrder("desc");
  });
  applyEntrySortUi();
